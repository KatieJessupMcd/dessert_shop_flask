...
class FlaskTests(TestCase):
    ...
    def text_post_desserts(self):
        """Make sure that the post request succeeds"""

        result = self.client.post('/desserts', json={
            "name": "Cookie",
            "description": "yummy",
            "calories": 3 
        })
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(result.json['name'], 'Cookie')
        self.assertEqual(result.json['description'], 'yummy')
        self.assertEqual(result.json['calories'], 3)
